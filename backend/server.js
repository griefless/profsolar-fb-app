...
app.get('/api/analytics', async (req, res) => {
    ...
    try {
        ...
        // 2. Daily insights — fetch each metric separately to avoid 400 on unavailable metrics
        let metrics = [];
        const metricGroups = [
            ['page_impressions_unique', 'page_impressions'],
            ['page_engaged_users'],
            ['page_fan_adds'],
            ['page_fan_removes'],
            ['page_video_views'],
        ...
        for (const group of metricGroups) {
            try {
                ...
                console.log(`✅ Metrics loaded: ${group.join(', ')}`);
            } catch (insightErr) {
                const errMsg = insightErr.response?.data?.error?.message || insightErr.message;
                console.warn(`⚠️ Skipping [${group.join(', ')}]: ${errMsg}`);
            }
        }
